...
export function RecurringModule({ schedules, categories, properties, projects, accounts, settings }: any) {
  ...
  return <div className="space-y-8">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
      <div><h1 className="text-4xl font-extrabold tracking-tight text-slate-900">Recurring</h1><p className="mt-1 text-slate-500">Manage subscriptions, bills, income, and repeating transactions.</p></div>
      <ScheduleDialog categories={categories} properties={properties} projects={projects} accounts={accounts} trigger={<Button className="rounded-full bg-indigo-600 px-6 hover:bg-indigo-700">Add Recurring +</Button>} />
    </div>
    <div className="grid gap-4 md:grid-cols-3">
      <Card><CardHeader><CardDescription>Total schedules</CardDescription><CardTitle className="text-3xl">{schedules.length}</CardTitle></CardHeader></Card>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      {filtered.map((schedule: any) => <Card key={schedule.id} className={cn("border-0 shadow-lg", !schedule.isActive && "opacity-75")}>
        <CardHeader><div className="flex items-start justify-between gap-3"><div><div className="mb-2 flex flex-wrap items-center gap-2"><span className={cn("rounded-full px-2 py-1 text-xs font-black", schedule.type === "INCOME" ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700")}>{schedule.type === "INCOME" ? "↑ MONEY IN" : "↓ MONEY OUT"}</span><span className={schedule.isActive ? "rounded-full bg-indigo-50 px-2 py-1 text-xs font-bold text-indigo-700" : "rounded-full bg-amber-50 px-2 py-1 text-xs font-bold text-amber-700"}>{schedule.isActive ? "ACTIVE" : "PAUSED"}</span></div><CardTitle>{schedule.name}</CardTitle><CardDescription>Every {schedule.interval} {frequencyUnit(schedule.frequency)}{schedule.interval > 1 ? "s" : ""}</CardDescription></div></div></CardHeader>
